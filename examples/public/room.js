  ...
  createRooms: function () {
    ...
    for (let x = 0; x < mapSource.height; x++) {
      for (let y = 0; y < mapSource.width; y++) {
        ...
        const roofPos = `${
          (x - mapSource.width / 2) * WALL_SIZE
        } ${WALL_HEIGHT} ${(y - mapSource.height / 2) * WALL_SIZE}`;
        const floorPos = `${(x - mapSource.width / 2) * WALL_SIZE} 0 ${
          (y - mapSource.height / 2) * WALL_SIZE
        }`;
        const position = `${(x - mapSource.width / 2) * WALL_SIZE} ${
          WALL_HEIGHT / 2
        } ${(y - mapSource.height / 2) * WALL_SIZE}`;
        const halfYposition = `${(x - mapSource.width / 2) * WALL_SIZE} ${
          WALL_HEIGHT / 4
        } ${(y - mapSource.height / 2) * WALL_SIZE}`;
        const quarterYposition = `${(x - mapSource.width / 2) * WALL_SIZE} ${
          WALL_HEIGHT / 8
        } ${(y - mapSource.height / 2) * WALL_SIZE}`;
